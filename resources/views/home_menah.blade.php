...
@section('content')
    <nav aria-label="breadcrumb" class="paths-nav">
        <ol class="breadcrumb breadcrumb-inverse">
            ...
            <li class="breadcrumb-item active" aria-current="page">ادارة الطلبات الخاصة بالمنح</li>
        </ol>
    </nav>
    <div class="row">
        <div class="col-lg-12">
                    ...
                    <table class="table table-hover ">
                        ...
                        @foreach($applications as $application)
                            <tr>
                                ...
                                @if($application->status === 'pending')
                                    <form action="{{route('application.status.accept',[$application->id,'menah'])}}" id="item-{{$application->id}}-accept" method="POST">@csrf</form>
                                    <form action="{{route('application.status.refuse',[$application->id,'menah'])}}" id="item-{{$application->id}}-refuse" method="POST">@csrf</form>

                                @elseif($application->status === 'canceled')
                                    <form action="{{route('application.destroy',[$application->id,'menah'])}}" id="item-{{$application->id}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                @else
